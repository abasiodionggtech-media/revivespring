...
// safeSendOtpEmail — never throws, always resolves (safe to fire-and-forget)
async function safeSendOtpEmail(toEmail, otp, language) {
  try {
    await sendOtpEmail(toEmail, otp, language);
  } catch (err) {
    console.error(`[EMAIL] safeSendOtpEmail swallowed error for ${toEmail}:`, err.message);
  }
}

module.exports = { sendOtpEmail, safeSendOtpEmail };            resolve(parsed);
          } else {
            reject(new Error(`Resend error ${res.statusCode}: ${data}`));
          }
        } catch {
